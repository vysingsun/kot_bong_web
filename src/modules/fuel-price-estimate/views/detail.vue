...
const onEditFileChange = (e: Event) => {
    ...
    const toAdd = Array.from(input.files ?? []).slice(0, MAX_IMAGES - editExistingImages.value.length - editFiles.value.length)
    ...
    <div class="flex flex-col h-screen bg-gray-100 dark:bg-gray-900">
        ...
        <div class="shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm z-10">
            <div class="max-w-2xl mx-auto flex items-center gap-3 px-4 py-3">
                <button class="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 transition-colors" @click="router.push('/fuel-price-estimate')">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </button>
                <div class="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-primary-hover flex items-center justify-center shrink-0">
                ...
                <div v-if="isSuperAdmin && !isEditing && store.currentEstimate" class="flex items-center gap-1">
                    <button class="p-1.5 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" @click="startEdit">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </button>
                    <button class="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" @click="deleteEstimateModal = true">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <template v-if="store.currentEstimate">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="flex items-center gap-2 mb-1 px-1">
                            <div class="w-6 h-6 rounded-full bg-gradient-to-br from-primary to-primary-hover flex items-center justify-center shrink-0">
                            ...
                            <span class="text-xs text-gray-400 ml-auto">{{ t('fuel_price_estimate.channel_post') }}</span>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl rounded-tl-sm shadow-sm overflow-hidden border-primary/60">
                            ...
                            <template v-if="!isEditing">
                                <p v-if="store.currentEstimate.content" class="px-4 pt-3 pb-2 text-sm text-gray-800 dark:text-gray-200 leading-relaxed whitespace-pre-line">
                                    {{ store.currentEstimate.content }}
                                </p>
                                <div v-if="store.currentEstimate.images?.length" class="px-4 pb-3">
                                ...
                                <div class="flex items-center justify-between px-4 py-2 border-t border-gray-100 dark:border-gray-700/60">
                                    <span
                                        v-if="store.currentEstimate.updatedAt && new Date(store.currentEstimate.updatedAt).getTime() > new Date(store.currentEstimate.createdAt).getTime()"
                                    ...
                                    <span v-else />
                                    <span class="text-xs text-gray-400 tabular-nums">{{ formatDateTime(store.currentEstimate.createdAt) }}</span>
                                </div>
                            </template>
                            ...
                            <template v-else>
                                <div class="px-4 py-4 space-y-3">
                                    ...
                                    <div v-if="editExistingImages.length" class="flex flex-wrap gap-2">
                                        <div v-for="img in editExistingImages" :key="img.publicId" class="relative group w-16 h-16">
                                            <img :src="img.url" class="w-full h-full object-cover rounded-lg" />
                                            <button class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100" @click="removeExistingImage(img.publicId)">×</button>
                                        </div>
                                    </div>
                                    <!-- New previews -->
                                    <div v-if="editPreviews.length" class="flex flex-wrap gap-2">
                                        <div v-for="(src, i) in editPreviews" :key="i" class="relative group w-16 h-16">
                                            ...
                                            <button class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100" @click="removeNewImage(i)">×</button>
                                        </div>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <label v-if="(editExistingImages.length + editFiles.length) < MAX_IMAGES" class="flex items-center gap-1.5 text-xs text-gray-400 hover:text-primary cursor-pointer transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                            {{ t('fuel_price_estimate.attach_images') }}
                                            <input type="file" accept="image/*" multiple class="hidden" @change="onEditFileChange" />
                                        </label>
                                        <div class="flex gap-2 ml-auto">
                                            <button class="px-3 py-1.5 text-xs rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700" @click="cancelEdit">{{ t('fuel_price_estimate.cancel') }}</button>
                                            <button :disabled="store.submitLoading" class="px-4 py-1.5 text-xs font-semibold text-white bg-primary hover:bg-primary-hover disabled:opacity-50 rounded-lg flex items-center gap-1" @click="submitEdit">
                                                <svg v-if="store.submitLoading" class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
                                                {{ store.submitLoading ? t('fuel_price_estimate.saving') : t('fuel_price_estimate.save') }}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="flex items-center gap-2 py-1">
                        ...
                        <span class="text-xs text-gray-400 shrink-0">{{ store.comments.length }} {{ t('fuel_price_estimate.comments') }}</span>
                        <div class="flex-1 h-px bg-gray-200 dark:bg-gray-700" />
                    </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 z-30 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-[0_-2px_12px_rgba(0,0,0,0.06)]">
            <div class="max-w-2xl mx-auto flex items-end gap-2 px-3 py-3">
                <textarea
        ...
        <!-- Lightbox -->
        <Teleport to="body">
            <div v-if="lightboxSrc" class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm" @click="lightboxSrc = ''">
                <img :src="lightboxSrc" class="max-w-[92vw] max-h-[88vh] rounded-xl object-contain" @click.stop />
                <button class="absolute top-4 right-4 w-9 h-9 bg-white/10 hover:bg-white/25 text-white rounded-full flex items-center justify-center" @click="lightboxSrc = ''">
        ...
        <!-- Modals -->
        <DeleteModal :show="deleteEstimateModal" :title="t('fuel_price_estimate.confirm_delete_title')" :description="t('fuel_price_estimate.confirm_delete_desc')" @close="deleteEstimateModal = false" @confirm="handleDeleteEstimate" />
        <DeleteModal :show="deleteCommentModal.show" :title="t('fuel_price_estimate.confirm_delete_comment_title')" :description="t('fuel_price_estimate.confirm_delete_comment_desc')" @close="deleteCommentModal.show = false" @confirm="handleDeleteComment" />
        <ErrorModal :show="errorModal.show" :description="errorModal.description" :error-message="errorModal.message" @confirm="errorModal.show = false" />
    </div>
</template>
