    ...
    <div class="flex flex-col h-screen bg-gray-100 dark:bg-gray-900">
        ...
        <div class="shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm z-10">
            <div class="max-w-2xl mx-auto flex items-center justify-between px-4 py-3">
                <div class="flex items-center gap-3">
                    <!-- Channel avatar -->
                    <div
                        class="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-primary-hover flex items-center justify-center shrink-0 shadow"
                    >
        ...
        <div ref="scrollContainer" class="flex-1 overflow-y-auto pb-24" @scroll="handleScroll">
            <div class="max-w-2xl mx-auto w-full px-3 py-4 space-y-4">
                ...
                <!-- Empty state -->
                <div
                    v-else-if="!store.loading && store.estimates.length === 0"
                    class="flex flex-col items-center justify-center py-24 text-center"
                >
                    <div
                        class="w-20 h-20 rounded-full bg-white dark:bg-gray-800 shadow flex items-center justify-center mb-4"
                    >
                        <svg
                    ...
                    <p class="text-gray-500 dark:text-gray-400 font-semibold">
                        {{ t('fuel_price_estimate.no_posts') }}
                    </p>
                    <p class="text-sm text-gray-400 dark:text-gray-500 mt-1 max-w-xs">
                        {{ t('fuel_price_estimate.no_posts_desc') }}
                    </p>
                </div>
                ...
                <template v-if="store.estimates.length > 0">
                    ...
                    <div v-for="est in displayEstimates" :key="est._id" class="space-y-4">
                        <!-- Others — left aligned -->
                        <div v-if="!isMe(est)" class="flex items-start gap-2.5">
                            <div
                                v-if="est.postedBy?.role?.role_name === 'Super_Admin'"
                            ...
                            <div class="flex flex-col gap-1 w-full max-w-xs md:max-w-md">
                                ...
                                >
                                    <div class="flex items-center space-x-2 mb-1 flex-wrap gap-y-1">
                                        <span class="text-sm font-semibold text-gray-900 dark:text-white"
                                            >{{ est.postedBy?.firstName }} {{ est.postedBy?.lastName }}</span
                                        >
                                        <span
                                            v-if="est.postedBy?.role?.role_name === 'Super_Admin'"
                                    ...
                                    <p class="text-sm text-gray-900 dark:text-white whitespace-pre-line line-clamp-5">
                                        {{ est.content }}
                                    </p>
                                    ...
                                    <div
                                        class="flex items-center justify-between mt-3 pt-2 border-t border-gray-200/60 dark:border-gray-700/60"
                                    >
                                        <button
                                        ...
                                        >
                                            ...
                                            <span
                                                >{{ est.commentCount ?? 0 }}
                                                {{ t('fuel_price_estimate.comments') }}</span
                                            >
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div v-if="isSuperAdmin" class="flex gap-3 ms-1">
                                    <button
                                        ...
                                        @click.stop="router.push(`/fuel-price-estimate/${est._id}`)"
                                    >
                                        {{ t('fuel_price_estimate.edit') }}
                                    </button>
                                    <button
                                        class="text-xs text-gray-400 hover:text-red-500"
                                        @click.stop="onDeleteClick(est)"
                                    >
                                        {{ t('fuel_price_estimate.delete') }}
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else class="flex items-start gap-2.5 justify-end">
                            <div class="flex flex-col gap-1 w-full max-w-xs md:max-w-md items-end">
                                ...
                                >
                                    ...
                                    <div
                                        class="flex items-center justify-between mt-3 pt-2 border-t border-blue-500/60"
                                    >
                                        <button
        ...
        >
            <div v-if="isSuperAdmin" class="max-w-2xl mx-auto px-3 py-3 space-y-2">
                ...
                <div
                    ...
                    class="flex flex-wrap gap-2 pb-2 border-b border-gray-150 dark:border-gray-700"
                >
                    <div v-for="(src, i) in previews" :key="i" class="relative group w-16 h-16 shrink-0">
                        <img
                            :src="src"
